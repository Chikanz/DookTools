...
namespace SpaceNavigatorDriver {
	...
	class ViewportController {
		...
		private static HashSet<Transform> transformsMoved = new HashSet<Transform>();
		...
		[Shortcut("SpaceMouse.Orbit")]
		public static void OrbitMode()
		{
			Settings.Mode = OperationMode.Orbit;
			Settings.Write();
		}
		
		[Shortcut("SpaceMouse.MoveMode")]
		public static void MoveMode()
		{
			Settings.Mode = OperationMode.Telekinesis;
			Settings.CoordSys = CoordinateSystem.Camera;
			Settings.ManipulateTranslationLock.All = false;
			Settings.ManipulateRotationLock.All = true;
			Settings.Write();
		}

		[Shortcut("SpaceMouse.RotateMode")]
		public static void RotateMode()
		{
			Settings.Mode = OperationMode.Telekinesis;
			Settings.CoordSys = CoordinateSystem.Local;
			Settings.ManipulateTranslationLock.All = true;
			Settings.ManipulateRotationLock.All = false;
			Settings.Write();
		}
		...
		static void Fly(SceneView sceneView, Vector3 translationInversion, Vector3 rotationInversion) {
			...
			Vector3 translation = Vector3.Scale(SpaceNavigator.Translation, translationInversion) * (float)editorDeltaTime * 100;
		...
		static void Telekinesis(SceneView sceneView) {
			...
			foreach (Transform selected in Selection.GetTransforms(SelectionMode.TopLevel | SelectionMode.Editable)) 
			{
				if (!_unsnappedRotations.ContainsKey(selected)) continue;
				...
				if (reference == null) {
					...
					_unsnappedTranslations[selected] += translation;
					_unsnappedRotations[selected] = rotation * _unsnappedRotations[selected];
				} else {
					// Move the object in the reference coordinate system.
					Vector3 worldTranslation = reference.TransformPoint(translation) -
											   reference.position;
					_unsnappedTranslations[selected] += worldTranslation;
					_unsnappedRotations[selected] = (reference.rotation * rotation * Quaternion.Inverse(reference.rotation)) * _unsnappedRotations[selected];
				}

				if (!transformsMoved.Contains(selected)) //record before moving
				{
					Undo.RecordObject(selected, "Move object");
					transformsMoved.Add(selected);
				}
				
				// Perform rotation with or without snapping.
				selected.rotation = Settings.SnapRotation ? SnapOnRotation(_unsnappedRotations[selected], Settings.SnapAngle) : _unsnappedRotations[selected];
				selected.position = Settings.SnapTranslation ? SnapOnTranslation(_unsnappedTranslations[selected], Settings.SnapDistance) : _unsnappedTranslations[selected];
			}
		}
